...
void AssetManager::loadModel(const char* modelPath)
{
    ...
    RDE_ASSERT_0(
        tinyobj::LoadObj(&attrib, &shapes, &materials, &warn, &err, modelPath),
        warn + err);
    ...
    RDE_LOG_INFO("Loaded model {0} with ID {1}",
...
void AssetManager::loadTexture(const char* texturePath)
{
    ...
    RDE_LOG_INFO("Loaded texture {0} with ID {1}",
                 m_assetNames[m_assetIDs[texturePath]],
